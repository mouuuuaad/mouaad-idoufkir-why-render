...
describe('getChanges advanced strategies', () => {
  ...
  it('fast-deep: ignores differences beyond default depth', () => {
    ...
    // Level 2: b
    // Level 3: c
    // Level 4: d -> difference here should be ignored?
    // Actually, fastDeepEqual returns true if depth < 0.
    // fastDeepEqual(a, b, 3) calls (a.b, b.b, 2) calls (a.b.c, b.b.c, 1) calls (a.b.c.d, b.b.c.d, 0).
    // At depth 0, it compares keys. If keys match, it calls (val, val, -1) which returns true.
    // So it effectively checks equality up to depth 3.
    ...
    // depth 0: keys(prev.a.b.c) = ['d']. loop 'd'. recurse(1, 2, -1) -> returns true!
    // So changes at depth 4 (d) are ignored.
    
    const changes = getChanges(prev, next, 'fast-deep');
    // Should be empty because it thinks they are equal
